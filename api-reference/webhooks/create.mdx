...
{
  "success": false,
  "error": "Invalid events: invalid.event. Valid: email.received, email.sent, email.delivered, email.bounced, email.spam, email.rejected, email.deleted, domain.verified"
}
```
...
Use the secret to verify that webhook requests are from Jasni. The signature is in the `X-Jasni-Signature` header with format `t=timestamp,v1=signature`:
