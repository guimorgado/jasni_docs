...
def handle_webhook():
    signature = request.headers.get("X-Jasni-Signature")
    payload = request.data  # Use raw body, not request.json
    ...
    elif event["event"] == "email.sent":
        print(f"Email sent to: {event['data']['to']}")
    elif event["event"] == "email.bounced":
        print(f"Email bounced: {event['data']['bounceReason']}")
...
```python
{
    "event": "email.received",     # Event type
    "timestamp": "2024-01-15T10:00:00Z",  # ISO timestamp when event occurred
    "data": {
        # Fields vary by event type
        # For email events:
        "id": "12345",                   # Email ID
        "messageId": "<msg@example.com>",  # RFC 2822 Message-ID
        "account": "me@jasnimail.com",   # Email account
