...
```typescript
try {
  await jasni.emails.list({ account: 'me@jasnimail.com' })
} catch (error) {
...
```typescript
try {
  await jasni.emails.get(999999, { account: 'me@jasnimail.com' })
} catch (error) {
...
try {
  const { email } = await jasni.emails.get(123, { account: 'me@jasnimail.com' })
